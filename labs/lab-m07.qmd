...
We can configure this with the `doMC` package, setting cores to 8 to match how I've set up your posit cloud instances. Relevant `tidymodels` functions will then automatically use parallelization as defined here by default. 
...
Extract the best-performing workflow from your workflow set and re-train it on the full training data. You can use the function `select_best(..., metric = "rmse")` on your fitted results to get the workflow with the best model and hyperparameter settings. 

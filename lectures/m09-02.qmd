...
#| echo: true
set.seed(1)
penguins_ = penguins %>% mutate_if(is.numeric, (function(x) scale(x, center=F)))
...
```{r}
#| echo: true
penguins_cv <- vfold_cv(penguins_, v = 5)
...
penguins_rec <- recipe(~ bill_length_mm + bill_depth_mm + flipper_length_mm + body_mass_g,
  data = penguins_
)
...
hc_clusters <- cbind(penguins_, extract_cluster_assignment(hc_fit))
```
...
the_bars <- cbind(
  sapply(cut(penguins_$bill_length_mm,breaks=c(0,unname(quantile(penguins_$bill_length_mm))), labels=heat_hcl(5)), color.id),
  sapply(cut(penguins_$bill_depth_mm,breaks=c(0,unname(quantile(penguins_$bill_depth_mm))), labels=heat_hcl(5)), color.id),
  sapply(cut(penguins_$flipper_length_mm,breaks=c(0,unname(quantile(penguins_$flipper_length_mm))), labels=heat_hcl(5)), color.id),
  sapply(cut(penguins_$body_mass_g,breaks=c(0,unname(quantile(penguins_$body_mass_g))), labels=heat_hcl(5)), color.id),
...
km_clusters <- cbind(penguins_, extract_cluster_assignment(kmeans_fit))
```
...
res.km <- kmeans(penguins_ %>% select(-c(sex, species, island)), 3, nstart = 25)

fviz_cluster(res.km, data = penguins_ %>% select(-c(sex, species, island)),
...
```{r}
#| echo: true
clusters <- km_clusters %>% rename(.km_cluster = .cluster) %>% transform(.hc_cluster = hc_clusters$.cluster)

conf_mat(clusters, truth=.hc_cluster, estimate=.km_cluster)
```
...
```{r}
#| echo: true
pca_rec <- recipe(~ bill_length_mm + bill_depth_mm + flipper_length_mm + body_mass_g, clusters) %>% 
  step_pca(all_predictors(), num_comp = 3)

pca_estimates <- prep(pca_rec, clusters)

pca_data <- cbind(clusters, bake(pca_estimates, clusters))
```
:::
...
```{r}
#| echo: true
pca_rec <- recipe(~ bill_length_mm + bill_depth_mm + flipper_length_mm + body_mass_g, penguins_) %>% 
  step_pca(all_predictors())

pca_estimates <- prep(pca_rec, penguins_)

pca_data <- cbind(penguins_, bake(pca_estimates, penguins_))
...
km_clusters <- cbind(pca_data, extract_cluster_assignment(kmeans_fit))
```
:::
...
```{r}
ggplot(km_clusters, aes(x = species, fill=.cluster)) +
...
```{r}
ggplot(km_clusters, aes(x = island, fill=.cluster)) +
  geom_bar(position="stack",stat="count")
```
...
```{r}
ggplot(km_clusters, aes(x = sex, fill=.cluster)) +
  geom_bar(position="stack",stat="count")
```
...
```{r}
ggplot(km_clusters, aes(x = bill_length_mm, fill=.cluster)) +
...
```{r}
ggplot(km_clusters, aes(x = bill_depth_mm, fill=.cluster)) +
  geom_histogram(position="identity",alpha=.7)
```
...
```{r}
ggplot(km_clusters, aes(x = flipper_length_mm, fill=.cluster)) +
  geom_histogram(position="identity",alpha=.7)
```
...
```{r}
ggplot(km_clusters, aes(x = body_mass_g, fill=.cluster)) +
  geom_histogram(position="identity",alpha=.7)
```
:::

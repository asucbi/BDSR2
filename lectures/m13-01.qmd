...
## ["Backpropagation through time" (BPTT) by unrolling]{style="font-size:75%;"}
...
- RNNs can handle sequences of varying length, but the ratio of input length to output length is constant
...
:::{style="font-size:70%;"}
- RNNs can handle sequences of varying length, but the ratio of input length to output length is constant
- For some applications, such as translation and question-answering, we might want to allow the output to vary in length
:::
...
## [Transformers: Multi-head & Cross Attention]{style="font-size:80%;"}
...
- Rather than processing inputs one at a time, Transformer models process an entire sequence in parallel by applying a *positional encoding*
...
## [Is human language computationally intractable?]{style="font-size:70%;"}


![[Van Rooij, et al. (2024). Reclaiming AI as a theoretical tool for cognitive science. Computational Brain & Behavior, 7(4), 616-636.]{style="font-size:70%;"}](../imgs/intractability.png)
